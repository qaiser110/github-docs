...
let promise1, promise2, promise3, momsPromise, promise4
...
momsPromise = new Promise(function(resolve, reject) {
  momsSavings = 20000;
  priceOfPhone = 60000;
...
promise3 = Promise.reject("Not interested");
...
let handleAllPromises = Promise.all(promisesArray);
...
handleAllPromises = Promise.all(promisesArray);
handleAllPromises.then(function(values) {
  console.timeEnd("Promise.All");
  console.log("All the promises are resolved", values);
});
...
let promisesRace = Promise.race(promisesArray);
...
promisesRace = Promise.race(promisesArray);
promisesRace.then(function(values) {
  console.timeEnd("Promise.race");
  console.log("The fasted promise resolved", values);
});
